...
class Seed:
    ...
    def loc_to_seed(loc: int) -> int:
        ...
        for m in list(maps.keys())[::-1]:
            for map in maps[m]:
                if map["dest_start"] <= src_attribute < map["dest_start"] + map["length"]:
                    src_attribute = map["src_start"] + (src_attribute - map["dest_start"])
                    break
...
class Day5(Puzzle):
    ...
    def __init__(self, data=None):
        ...
        for map in self.data[1:]:
            m = map.split(" map:\n")
            maps[m[0]] = [{"dest_start": int(d.group(1)), "src_start": int(d.group(2)), "length": int(d.group(3))} for d in regex.finditer(m[1])]
